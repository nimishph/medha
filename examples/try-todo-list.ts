/**
 * Smoke-drive the Sage engine against a real repo.
 *
...
for (const file of files) {
  await sage.record(fileKey(file), 'SKIP', { now: at() }, { ensure: true });
  const count = commitsTouching(file);
  applies.set(relPath(file), count);
  for (let i = 0; i < count; i++) await sage.record(fileKey(file), 'ADOPTED', { now: at() });
}

const ctx = { now: at(), seed: 42 };

const hints = await sage.hints(keys, ctx);
...
const candidates = await sage.explore(keys, { slots: 5, seed: 42 }, ctx);
...
const page = await sage.list({}, ctx, { limit: 5 });
...
const sim = await sage.simulate(simTarget.key, 'ADOPTED', ctx);
...
const detail = await sage.show(topId, ctx, { recent: 3 });
